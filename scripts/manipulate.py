...
def manipulate_ldo_2021(text):
    ...
    # first clean
    clear_lines = ['','Governo do Estado de São Paulo','\x0c002458969185137','\x0cProjeto de Lei de Diretrizes Orçamentárias 2021','META 2021']
    ...
    ldo_lines_clean = [line for line in ldo_lines_clean if line not in ['002458969185137']]
    ...
    for i in range(len(ldo_clean)):
        ...
        if i<len(ldo_clean)-3:
            if ldo_clean[i]=='AÇÃO':
                ldo_cleaned.append(ldo_clean[i] + " | " + ldo_clean[i+1] + " | " + ldo_clean[i+2])
    ...
    ldo_cleaned = [line for line in ldo_cleaned if line not in lines_to_remove+trash_lines]
